...
Step 11 is a hard blocker after Step 10 and before Step 12 trust/authority work, final copy, images, page packet, commit, deploy, or publish. It optimizes the first draft for on-page SEO inside the frozen Step 9/10 structure. Save the machine contract as `on-page-seo-optimized-draft.json`, the human review as `on-page-seo-optimized-draft.md`, and freeze `onPageSeoHash`.

`onPageSeoOptimizedDraft` must carry `step0AHash`, `step0BHash`, `pageJobHash`, `searchIntentHash`, `pageFormatHash`, `nextActionHash`, `serpCompetitorHash`, `topicResearchHash`, `uniqueAngleHash`, `contentBriefHash`, `pageOutlineHash`, and `firstDraftHash`. Missing or stale hashes are hard blockers.

The optimized draft must include `onPageSeoSummaryStatement`, `optimizedDraft`, `seoChangeLog`, `unresolvedOwnerStepItems`, `intentAlignmentCheck`, `h1OptimizationCheck`, `introOptimizationGate`, `headingOptimizationCheck`, `topicalCompletenessMap`, `naturalQueryCoverageContract`, `sectionRelevanceCheck`, `contentFocusChangeLog`, `internalLinkOptimization`, `assetOptimization`, `readabilityScanabilityCheck`, `answerPlacementCheck`, `naturalLanguageOptimizationGate`, `lightClaimSourceUseCheck`, `metadataCandidates`, `onPageSeoUniquenessCheck`, `onPageSeoDeliveryProofRequirements`, `mustCarryForward`, `step11OutputMustNotContain`, `step11CompletenessChecklist`, `step11RepairLog`, and `onPageSeoVerdict`.

Step 11 may improve H1, introduction, H2/H3 wording, section copy, answer placement, readability, natural keyword coverage, validated internal-link context, asset alt/fallback notes, and missing topical coverage already required or clearly implied by Steps 5, 6, 8, 9, or 10. It may increase, move, or remove off-intent, repetitive, over-covered, generic, or scope-drifting content, but it must not reduce required depth. Structural conflicts return to Step 9. New factual research needs return to Step 6. Brief/depth contract problems return to Step 8. Incomplete first-draft obligations return to Step 10.

`topicalCompletenessMap` must include required topic or reader need, current coverage, depth needed, status, action, upstream refs, evidence refs when relevant, and delivery proof. Actions are `add`, `expand`, `increase`, `move`, `remove`, `link`, `keep`, `return_to_step6`, `return_to_step8`, or `return_to_step9`. A page cannot pass if the main answer remains buried, required coverage is missing, or the map is generic.

`naturalQueryCoverageContract` must prove the target keyword and supporting queries are represented naturally in high-signal places. Keyword-density targets, awkward exact-match repetition, forced synonyms, robotic SEO language, scope expansion for keywords, and headings written only for keywords are forbidden.

`headingOptimizationCheck` must preserve the Step 9 section set and hierarchy. `internalLinkOptimization` may use only validated Step 4/8/site-inventory destinations and must preserve destination status. `assetOptimization` may improve context, alt text, captions, and text fallback notes, but must not create final image prompts or generated images. `lightClaimSourceUseCheck` must ensure Step 11 did not create unsupported claims, remove caveats, misuse audience-language evidence, or make risky claims more absolute.

`on-page-seo-optimized-draft.md` must maintain Markdown parity with `on-page-seo-optimized-draft.json` for optimized H1/H2/H3s, optimized section copy or meaningful before/after summary, topical-completeness map, keyword coverage, internal-link and asset notes, change log, unresolved items, summary statement, and verdict. JSON remains the gate.

`onPageSeoVerdict.status` may be `pass`, `pass_with_warnings`, `fail`, or `ask_user`; actions are `continue_to_step12`, `repair_step11`, `return_to_step10`, `return_to_step9`, `return_to_step8`, `return_to_step6`, `return_to_step5`, `ask_user`, or `skip_page`. Allow up to 2 repairs for weak Step 11 fields only. Do not repair true intent mismatch, structural conflicts, new research needs, missing Step 8/10 obligations, unsupported risky claims, current-batch duplication, missing Markdown parity, or no defensible optimized draft.

`mustCarryForward` must include `onPageSeoHash`, optimized H1, optimized heading map, topical-completeness actions delivered, natural query coverage, internal-link notes, asset-alt/fallback notes, candidate metadata, unresolved owner-step items, and proof requirements. Step 12 and final QA must include `onPageSeoDeliveryProof` proving they preserved or intentionally improved optimized H1/heading logic, answer placement, topical-completeness fixes, natural query coverage, internal-link context, asset alt/fallback notes, and anti-over-optimization rules.
