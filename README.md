...
After Step 10 and before Step 12 trust/authority work, final copy, images, commit, deploy, or publishing, the agent must complete Step 11 from `workflows/13-cluster-strategy.md`. Step 11 creates `onPageSeoOptimizedDraft`, saves `on-page-seo-optimized-draft.json` and `on-page-seo-optimized-draft.md`, and freezes `onPageSeoHash`.

Step 11 optimizes the completed first draft inside the frozen Step 9/10 structure. It must carry all upstream hashes including `pageOutlineHash` and `firstDraftHash`, and must include `onPageSeoSummaryStatement`, `optimizedDraft`, `seoChangeLog`, `unresolvedOwnerStepItems`, `intentAlignmentCheck`, `h1OptimizationCheck`, `introOptimizationGate`, `headingOptimizationCheck`, `topicalCompletenessMap`, `naturalQueryCoverageContract`, `sectionRelevanceCheck`, `contentFocusChangeLog`, `internalLinkOptimization`, `assetOptimization`, `readabilityScanabilityCheck`, `answerPlacementCheck`, `naturalLanguageOptimizationGate`, `lightClaimSourceUseCheck`, `metadataCandidates`, `onPageSeoUniquenessCheck`, `onPageSeoDeliveryProofRequirements`, `mustCarryForward`, `step11OutputMustNotContain`, `step11CompletenessChecklist`, and `onPageSeoVerdict`.

Step 11 may improve H1, introduction, H2/H3 wording, section copy, answer placement, readability, natural keyword coverage, validated internal-link context, asset alt/fallback notes, and missing topical coverage that is already required or clearly implied by Steps 5, 6, 8, 9, or 10. It may increase, move, or remove off-intent, repetitive, over-covered, generic, or scope-drifting content, but it must not reduce required depth. Structural problems return to Step 9; new factual research needs return to Step 6; brief/depth contract problems return to Step 8; incomplete draft obligations return to Step 10.

Step 11 must not finalize title tags, meta descriptions, schema, citation display, image prompts, generated images, technical SEO QA, live URL checks, or publishing decisions. Metadata fields are non-final candidates only. Keyword-density targets, keyword stuffing, awkward exact-match repetition, forced synonyms, and headings written only for keywords are forbidden. `on-page-seo-optimized-draft.md` must preserve Markdown parity with JSON for optimized headings/copy, topical-completeness map, keyword coverage, internal-link and asset notes, change log, unresolved items, verdict, and must-carry-forward summary.

Step 11 returns `continue_to_step12` only after `onPageSeoHash` is frozen. Step 12 and final QA must carry `onPageSeoHash` and include `onPageSeoDeliveryProof` showing optimized H1/heading logic, answer placement, topical-completeness fixes, natural query coverage, internal-link context, asset alt/fallback notes, and anti-over-optimization rules were preserved or intentionally improved.
...
Each selected batch page must first pass Step 0B, Step 1, Step 2, Step 3, Step 4, Step 5, Step 6, Step 7, Step 8, Step 9, Step 10, and Step 11, receiving its own Page Scope Contract, Page Job Contract, Search Intent Contract, Page Format Contract, Next Action Contract, SERP Competitor Analysis, Topic Research Bank, Unique Angle And Information Gain contract, SEO Content Brief, SEO Page Outline, SEO First Draft, and On-Page SEO Optimized Draft. Batch mode must run duplicate and isolation checks across current-batch and historical pages before generation by comparing `targetKeyword`, `targetQueryIntent`, `primaryIntent`, `primaryReaderStage`, `pageType`, `pageScopeSummary`, `uniqueContribution`, `mustCover`, `mustNotCover`, `nextActionStatement`, `serpCompetitorSummaryStatement`, `researchSummaryStatement`, `primaryImprovementAngle`, `informationGainAssets`, `areasToExceed`, `contentBriefSummaryStatement`, `readerOutcomePromise`, `instructionRegistry`, `antiGenericContract`, `workingH1`, `pageFlowType`, `sectionSequenceRationale`, `outlineSections`, `faqPlan`, `assetPlacementPlan`, `draftSummaryStatement`, `draftSections`, `requiredAssetDelivery`, `faqDraftDelivery`, `ctaInternalLinkDelivery`, `antiGenericDraftGate`, `onPageSeoSummaryStatement`, `optimizedDraft`, `topicalCompletenessMap`, `naturalQueryCoverageContract`, `internalLinkOptimization`, `assetOptimization`, `answerPlacementCheck`, `naturalLanguageOptimizationGate`, `mustCarryForward`, and optional `componentOpportunityHint`. Pages that only differ by keyword wording, share the same body promise, or would reuse the same section plan, SERP competitor findings, topic research bank, unique-angle logic, SEO content brief, SEO page outline, SEO first draft, on-page SEO optimization logic, examples, mistakes, reader questions, or decision content must be merged, split, repaired, rerouted, or blocked before downstream work. In batch mode, create `current-page.lock` only after Step 5 passes with action `continue_to_step6`; run Steps 6, 7, 8, 9, 10, and 11 under that page lock and do not start Step 12 or downstream page work until Step 11 passes with action `continue_to_step12`.
