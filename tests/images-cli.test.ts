...
test("blocks image planning when a V2 page workspace has not passed depth validation", async () => {
  const cwd = await mkdtemp(path.join(os.tmpdir(), "seo-images-"));
  await writeConfig({
    workspace_path: ".seo-agent-workspace",
    default_market: "India",
  ...
  await assert.rejects(
    () => planImagesFromWorkspace({ clusterSlug: "acne-treatment", pageId: "P1", cwd }),
    /Page cannot advance to images until validate-depth passes/
  );
});
