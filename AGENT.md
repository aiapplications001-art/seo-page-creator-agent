...
-> Step 11 On-Page SEO Optimized Draft: H1/intro/headings, topical completeness, natural queries, links, assets, readability, onPageSeoHash
...
## Step 7 Unique Angle And Information Gain Gate

Step 7 is a hard synthesis gate after Step 6 and before Step 8, prewriting, structure, final copy, images, page packet, commit, deploy, or publish. It performs no new research. It creates `uniqueAngleInformationGain`, saves `unique-angle-information-gain.json` and `unique-angle-information-gain.md`, and freezes `uniqueAngleHash`.

The contract must include all upstream hashes plus `primaryImprovementAngle`, 1-3 `supportingImprovementAngles`, `step5GapRouting`, `step6OpportunityRouting`, `informationGainAssets`, `baselineToMatch`, `areasToExceed`, `informationGainStatement`, `differentiationStatement`, `whyThisPageDeservesToCompeteStatement`, `informationGainQualificationCheck`, `formatCompatibilityCheck`, `safetyAndBrandFitCheck`, `originalityContract`, `multiSurfaceEvidenceCoverage`, `uniqueAngleUniquenessCheck`, `mustCarryForward`, `step7OutputMustNotContain`, `step7CompletenessChecklist`, `uniqueAngleRepairLog`, and `uniqueAngleVerdict`.

Every page needs one mandatory primary asset, one mandatory supporting asset, and one conditional optional supporting asset. Every asset needs `accessibleContentFallback`. The primary asset requires at least 2 Step 5 refs and 3 Step 6 evidence items from at least 2 credible sources; the mandatory supporting asset needs 2 Step 6 items plus Step 5 or audience/search evidence. The primary asset must pass the evidence + synthesis + action test and map to a top intent dimension and Step 2 satisfaction condition.

Require at least 2 `baselineToMatch` items and 5-7 distinct `areasToExceed`. Collectively prove primary SERP, credible topic research, secondary-query/long-tail, and audience-language coverage when available. Every Step 5 gap and relevant Step 6 opportunity must be explicitly routed. Mandatory assets must pass `formatCompatibilityCheck` and `safetyAndBrandFitCheck`.
...
Step 11 is the hard on-page optimization gate after Step 10 and before Step 12 trust/authority work, final copy, images, page packet, commit, deploy, or publish. It creates `onPageSeoOptimizedDraft`, saves `on-page-seo-optimized-draft.json` and `on-page-seo-optimized-draft.md`, freezes `onPageSeoHash`, and blocks Step 12 until `onPageSeoVerdict.action` is `continue_to_step12`.

Step 11 optimizes the completed draft inside the frozen `pageOutlineHash` and `firstDraftHash`. It may improve H1, introduction, H2/H3 wording, section copy, answer placement, readability, natural keyword coverage, validated internal-link context, asset alt/fallback notes, and missing topical coverage already required or clearly implied by Steps 5, 6, 8, 9, or 10. It may increase, move, or remove off-intent, repetitive, over-covered, generic, or scope-drifting content, but it must not reduce required depth. Structural conflicts return to Step 9; new research needs return to Step 6; brief/depth contract problems return to Step 8; incomplete first draft obligations return to Step 10.

Require all upstream hashes through `firstDraftHash`. The artifact must include `onPageSeoHash`, `onPageSeoSummaryStatement`, `optimizedDraft`, `seoChangeLog`, `unresolvedOwnerStepItems`, `intentAlignmentCheck`, `h1OptimizationCheck`, `introOptimizationGate`, `headingOptimizationCheck`, `topicalCompletenessMap`, `naturalQueryCoverageContract`, `sectionRelevanceCheck`, `contentFocusChangeLog`, `internalLinkOptimization`, `assetOptimization`, `readabilityScanabilityCheck`, `answerPlacementCheck`, `naturalLanguageOptimizationGate`, `lightClaimSourceUseCheck`, `metadataCandidates`, `onPageSeoUniquenessCheck`, `onPageSeoDeliveryProofRequirements`, `mustCarryForward`, `step11OutputMustNotContain`, `step11CompletenessChecklist`, `step11RepairLog`, and `onPageSeoVerdict`.

`topicalCompletenessMap` must map required topic/reader need, current coverage, depth needed, status, action, upstream refs, evidence refs when relevant, and optimized-draft delivery proof. Actions are `add`, `expand`, `increase`, `move`, `remove`, `link`, `keep`, `return_to_step6`, `return_to_step8`, or `return_to_step9`. Step 11 may not run new research; it exposes gaps and returns to the owner step when existing evidence is insufficient.

`naturalQueryCoverageContract` must prove the `targetKeyword` and supporting queries are represented naturally in high-signal places without keyword-density targets, awkward exact-match repetition, forced synonyms, robotic SEO language, or scope expansion. `headingOptimizationCheck` must preserve the Step 9 section set and hierarchy. `internalLinkOptimization` may use only validated destinations and must preserve destination status. `assetOptimization` may improve alt text, captions, context, and text fallback notes, but must not generate images or final image prompts.

`onPageSeoVerdict` may be `pass`, non-critical `pass_with_warnings`, `fail`, or `ask_user`, with action `continue_to_step12`, `repair_step11`, `return_to_step10`, `return_to_step9`, `return_to_step8`, `return_to_step6`, `return_to_step5`, `ask_user`, or `skip_page`. Step 11 may repair weak Step 11 fields up to 2 times, but true intent mismatch, structural conflict, missing research, missing Step 8/10 obligations, unsupported risky claims, current-batch duplication, missing Markdown parity, or no defensible optimized draft are blockers.

Step 11 must not output final title tag, final meta description, final schema, final citation display strategy, new research, unsupported new claims, final image prompts, generated images, technical SEO QA, live URL checks, publishing decisions, hidden Step 9 structure changes, keyword-density targets, or copied competitor/source wording. `metadataCandidates` are non-final only.

`on-page-seo-optimized-draft.md` must maintain strict Markdown parity with JSON for optimized headings/copy, topical-completeness map, keyword coverage, link and asset notes, change log, unresolved items, verdict, and must-carry-forward summary. `mustCarryForward` must give Step 12 and final QA `onPageSeoHash`, optimized H1, optimized heading map, topical-completeness actions delivered, natural query coverage, internal-link notes, asset-alt/fallback notes, candidate metadata, unresolved owner-step items, and proof requirements. Step 12 and final QA must include `onPageSeoDeliveryProof`.
